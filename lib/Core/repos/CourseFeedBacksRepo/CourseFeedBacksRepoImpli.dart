...
import 'package:sintir_dashboard/Core/repos/CourseFeedBacksRepo/CourseFeedBacksRepo.dart';
...
class CourseFeedBacksRepoImpli implements CourseFeedBacksRepo {
  ...
  Future<Either<Failure, void>> addCourseFedBack({
    ...
    try {
      ...
      return left(
        ServerFailure(message: "حدث خطأ غير متوقع أثناء إضافة التقييم"),
      );
    }
  }
  ...
  Future<Either<Failure, GetCourseFeedBacksResponseEntity>> getCourseFedBacks({
    ...
    try {
      ...
      if (response.listData == null) {
        return left(
          ServerFailure(message: "لا توجد تقييمات متاحة لهذه الدورة"),
        );
      }
      ...
      return left(
        ServerFailure(message: "فشل تحميل التقييمات، يرجى المحاولة لاحقاً"),
      );
    }
  }
}
...
List<CoursefeedbackItemEntity> _parseCourseFeedbacks(
  ...
  for (final Map<String, dynamic> item in rawList) {
    try {
      ...
      throw CustomException(message: "خطأ في معالجة بيانات التقييمات");
    }
  }
  return results;
}
